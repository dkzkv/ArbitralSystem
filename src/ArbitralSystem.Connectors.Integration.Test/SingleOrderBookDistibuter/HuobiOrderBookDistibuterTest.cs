...
{
    [TestClass]
    public class HuobiOrderBookDistributerTest : BaseOrderBookDistributerTest
    {
        [TestMethod]
        public async Task HuobiDistributionDefaultOptions()
        {
            var options = new DistributerOptions {Frequency = 100};

            var orderbooks = new List<IDistributorOrderBook>();

            var binanceDistributer = new HuobiOrderbookDistributor(options, DtoConverter, Logger);
            ...
            var orderBookPairInfo = new OrderBookPairInfo(Exchange.Huobi, pairInfo.ExchangePairName, pairInfo.UnificatedPairName);
            ...
            Assert.AreEqual(orderbook.Exchange, Exchange.Huobi);
